...
export default function CoordinatorFacultyPage() {
  ...
  const columns: DataTableColumn<FacultyListItem>[] = [
    { key: "id", header: "ID", width: "0.7fr", render: (f) => <span className="font-mono text-[11px] text-muted">FAC{f.id}</span> },
    {
      ...
      render: (f) => (
        <span
          className={`-mx-2 rounded px-2 py-1 font-semibold ${f.id === effectiveSelectedId ? "bg-accent-100 text-primary" : "text-ink"}`}
        >
          {f.name}
        </span>
      ),
          ...
          {!profile.data ? (
            ...
            <>
              ...
              <div className="mt-4">
                ...
                <ProgressBar percent={Math.min(100, Math.round((profile.data.weeklyLoadHours / profile.data.weeklyLoadCapHours) * 100))} height={8} className="mt-2" />
              </div>
            </>
          )}
        </Card>
