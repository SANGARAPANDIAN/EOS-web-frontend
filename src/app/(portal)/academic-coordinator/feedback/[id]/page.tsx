      ...
      {[5, 4, 3, 2, 1].map((v) => {
        ...
        return (
          <div key={v} className="flex items-center gap-2">
            ...
            <ProgressBar percent={(count / max) * 100} height={8} className="flex-1" />
      ...
      {q.question_type === "rating" ? (
        ...
        <div className="mt-2 flex max-h-40 flex-col gap-1.5 overflow-y-auto">
          {(q.responses ?? []).filter(Boolean).length === 0 ? (
            <EmptyState message="No text responses yet." className="py-1" />
          ) : (
            (q.responses ?? []).filter(Boolean).map((r, i) => (
              <div key={i} className="rounded-[8px] bg-surface-muted px-2.5 py-2 text-[12.5px] text-body">
                {r}
              </div>
            ))
          )}
  ...
  return (
    <div className="flex flex-col gap-4.5">
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div>
          <button
            ...
            className="mb-1.5 flex items-center gap-1 text-xs font-semibold text-primary hover:text-primary-dark"
          >
            <Icon name="arrow_back" size={14} /> All feedback forms
          </button>
          <h1 className="m-0 text-[24px] font-bold tracking-[-.02em] text-ink">{form.data.title}</h1>
        ...
        <Button variant="secondary" className="text-danger-fg border-danger-border" onClick={() => setConfirmingDeleteForm(true)}>
          Delete form
        </Button>
      </div>

      <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-3.5">
        <StatCard label="Target students" value={results.data?.target_student_count ?? "—"} />
        <StatCard label="Responses received" value={results.data?.respondent_count ?? "—"} />
      ...
      <Card>
        ...
        <div className="mt-2.5 flex flex-col">
          {form.data.questions.map((q) => (
            <div key={q.id} className="flex items-center gap-2.5 border-t border-divider py-2.5">
              <span className="w-[18px] text-xs text-subtle">{q.sequence_no}.</span>
              <span className="flex-1 text-[13px] text-ink">{q.question_text}</span>
              <Badge tone={q.question_type === "rating" ? "accent" : "neutral"}>
              ...
              {deletingQuestionId === q.id ? (
                ...
                <IconButton
                  ...
                  className="shrink-0 text-danger-fg hover:border-danger-border hover:bg-danger-bg"
                />
              )}
            </div>
          ))}
        </div>
        <div className="mt-3.5 flex gap-2 border-t border-border-default pt-3.5">
          <Input
          ...
          <Button variant="primarySmall" onClick={handleAddQuestion} disabled={addQuestion.isPending}>
      ...
      <Card>
        ...
        {results.isLoading && <EmptyState loading className="mt-2" />}
...
function MatrixResults({ results }: { results: FeedbackMatrixResults }) {
  ...
  if (results.rows.length === 0) {
    return <EmptyState message="No faculty roster found for this class yet." className="mt-2" />;
  }
  ...
  return (
    <div className="mt-2.5 flex flex-col">
      {results.rows.map((row) => {
        ...
        return (
          <div key={row.mapping_id} className="border-t border-divider">
            <div
