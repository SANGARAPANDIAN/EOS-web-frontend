  ...
  return (
    <div className="flex flex-col gap-4.5">
      ...
      <div className="grid grid-cols-[1fr_380px] items-start gap-4.5">
              ...
              return (
                ...
                >
                  <div className="mb-2.5 flex items-center justify-between">
                    ...
                    <span className="rounded-pill bg-accent-100 px-2 py-0.5 text-[11px] font-semibold text-primary">{bucket.mapped.length} mapped</span>
                  </div>

                  {bucket.mapped.length === 0 ? (
                    <div className="py-2.5 text-[11.5px] text-subtle">{isEmpty ? "No class sits here — nothing to map." : "Drag a course here to map it."}</div>
                  ) : (
                      ...
                      {bucket.mapped.map((s) => {
                        ...
                        return (
                          <span
                            ...
                            className={`inline-flex items-center gap-1.5 rounded-pill py-1 pl-2.5 pr-1.5 text-[11.5px] font-semibold ${
                              partial ? "bg-[#fef9c3] text-[#854d0e]" : "bg-surface-tint text-ink"
                            }`}
                          >
                            {s.subjectCode}
                            {partial && (
                            ...
                            <button
                              ...
                              className="grid size-4 cursor-pointer place-items-center rounded-full border-0 bg-black/10 text-[10px] leading-none text-inherit"
                            >
                              ×
                            </button>
                          </span>
        ...
        <div className="sticky top-4.5 flex max-h-[calc(100vh-140px)] flex-col rounded-card border border-border-default bg-surface p-3.5">
          <h2 className="m-0 mb-2.5 shrink-0 text-[15px] font-bold text-ink">Course pool</h2>
          <Input
            value={search}
          ...
          />
          <Select value={poolDeptFilter} onChange={(e) => setPoolDeptFilter(e.target.value)} className="mb-2.5 shrink-0">
            ...
            {pool.length === 0 ? (
              ...
              pool.map((s) => (
                <div
                  ...
                  className={`cursor-grab rounded-[8px] border border-border-default px-2.5 py-2 ${draggingId === s.id ? "bg-accent-100 opacity-50" : "bg-surface"}`}
                >
      ...
      <ConfirmDialog
        ...
        description={removing ? `${removing.subject.subjectCode} · ${removing.subject.name} will be unmapped from Semester ${removing.semester} in this department.` : ""}
        confirmLabel="Remove"
        destructive
