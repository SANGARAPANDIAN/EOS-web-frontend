...
function computeClashes(classSlots: TimetableSlot[], allSlots: TimetableSlot[]): ClashRow[] {
  ...
  for (const slot of classSlots) {
    ...
    for (const c of conflicting) {
      facultyClashes.push(
        `${slot.facultyName} — ${DAY_LABELS[slot.dayOfWeek]} P${slot.periodNumber}: also teaching ${c.departmentCode} ${c.classSection}`,
      );
    }
  }
      ...
      <Card className="overflow-hidden p-0">
        <div className="border-b border-divider px-5 py-4">
          ...
          <p className="mt-1 text-xs text-muted">
            {selectedClass ? `${deptCodeById.get(selectedClass.department_id) ?? "?"} · Sec ${selectedClass.section}` : ""} ·{" "}
            {periods.length} periods, Monday to Saturday.
          </p>
        </div>
        ...
        {slots.isLoading ? (
          ...
          <EmptyState message="No timetable slots published yet." />
        ) : (
          <div className="overflow-x-auto">
            <div className="grid min-w-[900px]" style={{ gridTemplateColumns: `110px repeat(${days.length}, minmax(140px,1fr))` }}>
              <div className="border-r border-b border-divider bg-surface-tint" />
              {days.map((d) => (
              ...
              {periods.map(({ period, start, end }) => (
                <Fragment key={period}>
                  <div className="border-r border-b border-divider px-3.5 py-2.5 text-[11px] text-muted">
                    {start} – {end}
                  </div>
                  {days.map((d) => {
                    ...
                    return (
                      <div
                        key={`${d}-${period}`}
                        className={`min-h-14 border-b border-divider px-2.5 py-2 ${cell ? "bg-accent-50" : "bg-surface"}`}
                      >
                        {cell && (
                          <>
      ...
      <Card>
        ...
        <div className="mt-3 flex flex-col gap-2">
          {clashes.map((c, i) => (
            <div
              key={i}
              className="flex items-center gap-2.5 rounded-input px-3.5 py-2.5"
              style={{
            ...
            >
              <span
                ...
                style={{ background: c.kind === "good" ? "#16a34a" : "#dc2626" }}
              />
              <span className="shrink-0 text-[12.5px] font-bold">{c.label}</span>
