...
import { Card, Button, Badge, Toggle, type BadgeTone } from "@/components/ui";
...
export default function CoeHallsSeatingPage() {
  ...
  const myProfile = useMyCoeProfile();
  ...
  // single exam. Prefers the semester filter when set, else the most
  // recent exam of the chosen type (within the chosen year, if any) in the
  ...
  const resolvedExam = useMemo(() => {
    ...
    if (semesterFilter !== "all") {
      return candidates.find((e) => e.semester === semesterFilter) ?? candidates.sort((a, b) => b.id - a.id)[0] ?? null;
    }
    return candidates.sort((a, b) => b.id - a.id)[0] ?? null;
  }, [exams.data, currentAcademicYear, effectiveExamTypeId, yearFilter, semesterFilter]);
  ...
  const overview = useSeatingOverview({ exam_id: effectiveExamId, exam_date: examDate, session });
  ...
  const targetVenueParams = selectedVenueId && effectiveExamId ? { exam_id: effectiveExamId, exam_date: examDate, session, venue_id: selectedVenueId } : null;
  ...
  const visibleVenues = useMemo(() => {
    ...
    return list.filter((v) => v.departments.some((d) => d.id === departmentFilter));
  ...
  return (
    <div className="flex flex-col gap-5 animate-pop-in">
      ...
      {tab !== "allocate" ? (
        <VersionsList status={tab} isSenior={myProfile.data?.is_senior ?? false} />
      ) : filtersLoading ? (
        <SkeletonFilterBar />
      ) : (
        <>
          <Card className="border-border-accent bg-accent-50">
            ...
            <div className="mt-1 text-[15px] font-extrabold text-ink">
              {resolvedExam ? `${examTypesById.get(resolvedExam.exam_type_id)?.name ?? "Exam"} · Semester ${resolvedExam.semester} · ${currentAcademicYear}` : "No examination resolved"}
              {departmentFilter !== "all" && ` · ${departments.data?.find((d) => d.id === departmentFilter)?.code ?? ""}`}
            </div>
            <div className="mt-0.5 text-[12.5px] text-primary-dark">
              {examDate || "No exam date selected"} · {session} session
            ...
            {overview.data && (
              <div className="mt-2.5 border-t border-border-accent pt-2.5 text-[12.5px] text-primary-dark">
                Mapped to {currentAcademicYear} / Semester {resolvedExam?.semester} / {examTypesById.get(effectiveExamTypeId ?? -1)?.name} /
                v{overview.data.version.version_number} · {overview.data.venues.length} venues · {overview.data.total_seated} of{" "}
                {overview.data.total_seats} seats filled
              </div>
            )}
          </Card>
            ...
            <div className="grid grid-cols-5 gap-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {(["FN", "AN"] as ExamSessionCode[]).map((s) => (
                    <button
                      key={s}
                      type="button"
                      onClick={() => {
                      ...
                      className={cn(
                        "flex-1 rounded-pill border px-3 py-2.5 text-[12.5px] font-bold transition-colors",
                        session === s ? "border-primary bg-accent-50 text-primary" : "border-border-default text-muted hover:text-ink",
                      )}
                    >
                      {s === "FN" ? "FN 10:00–13:00" : "AN 14:00–17:00"}
                    </button>
                  ))}
                </div>
              </div>
                ...
                {!selectedVenueId ? (
                  ...
                  <Card>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge tone={venueDetail.data.seats.length > 0 ? "accentDark" : "neutral"}>
                        {venueDetail.data.seats.length > 0 ? "SAVED TO DRAFT" : "NOT SEATED YET"}
                      </Badge>
                    </div>
                    ...
                    {venueDetail.data.allocation_mode !== "manual" ? (
                      <>
                        <div className="mt-4 text-[11px] font-extrabold uppercase tracking-[.08em] text-subtle">Departments allowed in this venue</div>
                        <div className="mt-2 flex flex-wrap gap-2">
                          {(departments.data ?? []).map((d) => {
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  "rounded-[8px] border px-3 py-1.5 text-[12.5px] font-bold transition-colors",
                        ...
                        {venueDetail.data.department_breakdown.length === 0 ? (
                          <div className="mt-4 flex items-center justify-between rounded-[10px] bg-surface-tint px-3.5 py-2.5">
                            <span className="text-[12.5px] font-bold text-ink">{venueDetail.data.candidates_waiting} candidates waiting to be seated</span>
                            <span className="text-[12px] text-muted">{venueDetail.data.seats.length} of {venueDetail.data.venue.capacity} seated</span>
                          </div>
                        ) : (
                        ...
                        <div className="mt-4 text-[11px] font-extrabold uppercase tracking-[.08em] text-subtle">Seating pattern</div>
                          ...
                          {Array.from({ length: venueDetail.data.venue.capacity }, (_, pos) => {
                            ...
                            return seat ? (
                              <div key={pos} className="rounded-[8px] bg-primary px-1.5 py-2 text-center text-white">
                                <div className="text-[9.5px] font-bold opacity-80">{seat.seat_number}</div>
                              ...
                              <div key={pos} className="rounded-[8px] border border-dashed border-border-default bg-surface-tint px-1.5 py-2 text-center">
                                <div className="text-[9.5px] font-bold text-subtle">—</div>
                              </div>
                            );
                          })}
                          </div>
...
function VersionsList({ status, isSenior }: { status: "draft" | "ready_to_publish" | "published"; isSenior: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-[12px] text-subtle">
        {status === "draft" ? "Saved allocations awaiting Senior COE verification" : status === "ready_to_publish" ? "Submitted, waiting for a Senior COE to publish" : "Live seating plans"}
      </p>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  {status === "ready_to_publish" &&
                    (isSenior ? (
                      ...
                      <span className="text-[12px] text-subtle">Only a Senior COE can publish</span>
                    ))}
                </div>
              </div>
            </Card>
