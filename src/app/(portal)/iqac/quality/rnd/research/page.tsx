...
export default function ResearchPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return allRows.filter((r) => {
      const okQ = !q || `${r.contributor.name} ${r.centre_name} ${r.focus_area ?? ""}`.toLowerCase().includes(q);
  ...
  return (
    <div className="flex flex-col gap-5 animate-pop-in">
      <MetricBackNav crumb="IQAC · Research & Development · Research" />
      <MetricHeader
        name="Research"
        blurb="Research centres, projects and investigator roles, held by faculty and/or students."
        addLabel="+ Add entry"
      ...
      <MetricFilterBar
        search={search}
        onSearch={setSearch}
        searchPlaceholder="Search contributor, centre or focus area"
        selects={[
          {
            label: "DEPARTMENT",
            value: dept ?? "all",
