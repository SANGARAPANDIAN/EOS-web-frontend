...
import {
  useMediaRoomAppraisalCriteria,
  useMediaRoomAppraisalHistory,
  useApplyMediaRoomAppraisal,
  type MediaRoomAppraisalDivision,
  type MediaRoomAppraisalHistoryRow,
} from "@/modules/media-room/api/employeeAppraisal";
...
function statusFooter(row: MediaRoomAppraisalHistoryRow): string {
  switch (row.status) {
    ...
    default:
      return "Submitted, awaiting HR review";
  }
}

export default function MediaRoomEmployeeAppraisalPage() {
  const [tab, setTab] = useState<"apply" | "history">("apply");
  const criteria = useMediaRoomAppraisalCriteria();

  return (
    <div className="flex flex-col gap-5 animate-pop-in">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[34px] font-extrabold tracking-[-.03em] text-ink">Request Appraisal</h1>
...
function ApplyForm() {
  const criteria = useMediaRoomAppraisalCriteria();
  const apply = useApplyMediaRoomAppraisal();
  ...
  function filledCount(division: MediaRoomAppraisalDivision): number {
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const entries = divisions.flatMap((d) => d.criteria.map((c) => ({ criteria_id: c.id, description: descriptions[c.id] || undefined })));
  ...
  return (
    <div className="flex flex-col gap-4">
      {submitted && <div className="rounded-[10px] bg-accent-50 px-4 py-3 text-[13px] font-bold text-primary">Appraisal request submitted.</div>}

      {divisions.map((division) => {
        const isOpen = expanded === division.id;
        const maxTotal = division.criteria.reduce((sum, c) => sum + c.max_score, 0);
        return (
          <Card data-mr-lift="1" key={division.id} className={isOpen ? "border-border-accent" : ""}>
            <button type="button" onClick={() => setExpanded(isOpen ? null : division.id)} className="flex w-full items-center justify-between gap-4">
                ...
                {division.criteria.map((c) => (
                  <div key={c.id}>
                    ...
                    <div className="mt-2 grid grid-cols-[110px_1fr] gap-3">
                      <Input type="number" max={c.max_score} placeholder="Score" disabled title="Scored by HR during review, not at submission" className="bg-surface-tint" />
                      <Input value={descriptions[c.id] ?? ""} onChange={(e) => setDescriptions((d) => ({ ...d, [c.id]: e.target.value }))} placeholder="Supporting remark or evidence" />
                    </div>
                  </div>
                ))}
              </div>
...
function HistoryList() {
  const history = useMediaRoomAppraisalHistory();

  const rows = useMemo(() => {
    return (history.data?.data ?? []).map((r) => {
  ...
  if (history.data && !history.data.ready) {
    return (
      <Card data-mr-lift="1">
        <div className="text-[13px] text-subtle">Appraisal requests aren&apos;t set up yet — ask an admin to run the pending database migration.</div>
      </Card>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      {rows.map(({ row, scored, totalScore, totalMax, grade }) => (
        <Card data-mr-lift="1" key={row.id}>
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="mt-1 text-[16px] font-extrabold text-ink">Academic Year {row.academic_year.replace("-", "–")}</div>
              <div className="mt-0.5 text-[13px] text-body">{scored ? `Score ${totalScore} / ${totalMax} · Grade ${grade}` : "Awaiting score"}</div>
            </div>
