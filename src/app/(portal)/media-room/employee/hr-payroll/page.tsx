...
import {
  useMediaRoomHrPayrollRequests,
  useCreateMediaRoomHrPayrollRequest,
  type MediaRoomHrPayrollRequestRow,
} from "@/modules/media-room/api/employeeHrPayroll";
...
export default function MediaRoomEmployeeHrPayrollPage() {
  return (
...
function RequestForm() {
  const create = useCreateMediaRoomHrPayrollRequest();
  ...
  return (
    <Card data-mr-lift="1">
      {submitted && <div className="mb-4 rounded-[10px] bg-accent-50 px-4 py-3 text-[13px] font-bold text-primary">Request submitted.</div>}
      ...
      <div className="mt-5">
        ...
        <Textarea rows={5} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe your request in detail" />
      </div>
      ...
      <Button variant="primary" className="mt-6" onClick={submit} disabled={category === CATEGORIES[0] || !subject || create.isPending}>
...
function RequestStatusList() {
  const requests = useMediaRoomHrPayrollRequests();
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {requests.isLoading ? (
        ...
        requests.data.map((r: MediaRoomHrPayrollRequestRow) => (
