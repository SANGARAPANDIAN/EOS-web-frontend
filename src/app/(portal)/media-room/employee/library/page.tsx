...
export default function MediaRoomEmployeeLibraryPage() {
  const [tab, setTab] = useState<Tab>("borrowed");
  const overview = useMediaRoomLibraryOverview();
  const o = overview.data;

  return (
    <div className="flex flex-col gap-5 animate-pop-in">
      <div>
        <h1 className="text-[34px] font-extrabold tracking-[-.03em] text-ink">Library</h1>
        <p className="mt-1 text-[13px] text-muted">{o ? `Central library · ${o.card_no} · ${o.borrowed.length} title(s) borrowed` : "Central library"}</p>
      </div>
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              className={cn("flex-1 rounded-[10px] py-2.5 text-[14px] font-bold transition-colors", active ? "bg-primary text-white" : "cursor-pointer text-ink hover:text-primary")}
            >
              {t.label}
            </button>
          );
...
function BorrowedTab({ overview }: { overview: ReturnType<typeof useMediaRoomLibraryOverview> }) {
  ...
  if (overview.data && !overview.data.ready) {
    return (
      <Card data-mr-lift="1">
        <div className="text-[13px] text-subtle">Staff book loans aren&apos;t set up yet — ask an admin to run the pending database migration.</div>
      </Card>
    );
  }
  if (rows.length === 0) {
          ...
          <Card data-mr-lift="1" key={r.id}>
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-0.5 truncate text-[13px] text-muted">{[r.book.author, r.book.qr_code].filter(Boolean).join(" · ")}</div>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {search.isLoading ? (
        ...
        <Card data-mr-lift="1">
          <div className="text-[13px] text-subtle">{q.trim().length === 0 ? "No books in the catalogue yet." : `No books matched "${q}".`}</div>
        </Card>
      ) : (
        <div className="flex flex-col gap-3">
          {rows.map((b) => {
            ...
            return (
              <Card data-mr-lift="1" key={b.id}>
                <div className="flex items-center gap-4">
                  <div className="size-12 shrink-0 rounded-[10px] bg-accent-50" />
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-[15px] font-extrabold text-ink">{b.title}</div>
                    <div className="mt-0.5 truncate text-[13px] text-muted">{[b.author, b.qr_code].filter(Boolean).join(" · ")}</div>
                    <div className="mt-0.5 text-[12px] text-subtle">{available ? `${b.available_copies} of ${b.total_copies} copies available` : "All copies issued"}</div>
                  </div>
                  <span
        ...
        <Card data-mr-lift="1" key={r.id}>
          <div className="flex items-center gap-4">
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 text-[13px] text-muted">{[r.category_name, r.format?.toUpperCase(), r.pages ? `${r.pages} pages` : null].filter(Boolean).join(" · ")}</div>
            </div>
...
function HistoryTab({ overview }: { overview: ReturnType<typeof useMediaRoomLibraryOverview> }) {
