...
import { useMediaRoomPayslipHistory, useApplyMediaRoomPayslip, type MediaRoomPayslipHistoryRow } from "@/modules/media-room/api/employeePayslip";
...
export default function MediaRoomEmployeePayslipPage() {
  const currentYear = new Date().getFullYear();
  const years = [currentYear, currentYear - 1, currentYear - 2];

  const history = useMediaRoomPayslipHistory();
  const apply = useApplyMediaRoomPayslip();
  ...
  async function submit() {
    setError(null);
    try {
      await apply.mutateAsync({ month: `${year}-${String(month + 1).padStart(2, "0")}`, purpose });
      setSubmitted(true);
      setRemarks("");
    } catch (err: unknown) {
  ...
  const columns: DataTableColumn<MediaRoomPayslipHistoryRow>[] = [
    {
      key: "ref",
      header: "Ref",
      width: "1fr",
      render: (r) => <span className="font-bold text-subtle">PSL-{r.month.slice(0, 4)}-{String(r.id).padStart(3, "0")}</span>,
    },
    ...
    { key: "status", header: "Status", width: "100px", render: (r) => <Badge tone={statusTone(r.status)}>{statusLabel(r.status)}</Badge> },
  ];

  return (
    <div className="flex flex-col gap-5 animate-pop-in">
      <div>
        <h1 className="text-[34px] font-extrabold tracking-[-.03em] text-ink">Payslip Request</h1>
      ...
      <div className="grid grid-cols-[1fr_1.5fr] items-start gap-5">
        <Card data-mr-lift="1">
          {submitted && <div className="mb-4 rounded-[10px] bg-accent-50 px-4 py-3 text-[13px] font-bold text-primary">Payslip request submitted.</div>}
          ...
          <div className="mt-5">
            ...
            <Textarea rows={4} value={remarks} onChange={(e) => setRemarks(e.target.value)} placeholder="Any additional detail for the HR team" />
          </div>

          {error && <div className="mt-4 text-[13px] font-semibold text-danger-fg">{error}</div>}
          <Button variant="primary" className="mt-6" onClick={submit} disabled={apply.isPending || !!notReady}>
        ...
        <div>
          ...
          {history.data && !history.data.ready ? (
            <div className="rounded-card border border-border-default bg-surface p-4 text-[13px] text-subtle">
              Payslip requests aren&apos;t set up yet — ask an admin to run the pending database migration.
            </div>
          ) : (
            <DataTable columns={columns} data={history.data?.data ?? []} rowKey={(r) => r.id} loading={history.isLoading} emptyMessage="No payslip requests yet." />
          )}
        </div>
      </div>
    </div>
