...
// migration) and `/attendance` (mark + read) modules — see
// `src/modules/secretary/api/attendance.ts` header comment for the two
// honest gaps this surfaces:
//   - The real attendance_status_enum this endpoint writes only supports
//     present/absent — there is no "OD" reachable through it. The
//     design's 3-way Present/Absent/OD toggle is genuinely 2-way here;
//     OD is dropped, not faked.
//   - There is no attendance changelog/audit-trail table anywhere in the
//     schema, so the design's "find a person, see who changed their mark"
//     History feature has no real backing. Replaced with a real
//     class+date attendance-records read instead.

const MONTH_NAMES = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
const MARK_TONE = { bgP: "#ecfdf5", fgP: "#047857", bgA: "#fef2f2", fgA: "#b91c1c" };
...
export default function SecretaryAttendancePage() {
  ...
  async function saveAttendance() {
    ...
    try {
      ...
      flash(`Attendance saved · ${present} present, ${absent} absent.`);
    } catch (err) {
  ...
  const { data: histRecords, isLoading: histLoading, error: histError } = useAttendanceRecords({ class_id: histClassId, date: histDate });
      ...
      {tab === "Mark" && (
        <>
          ...
          <div data-sec-lift="" style={{ background: "#ffffff", border: "1px solid #e5e9f2", borderRadius: 14, overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1.2fr 2fr 1fr 1.6fr", gap: 12, padding: "13px 20px", background: "#ffffff", borderBottom: "1px solid #eef2f7", fontSize: 11.3, fontWeight: 600, letterSpacing: 0.6, textTransform: "uppercase", color: "#94a3b8" }}>
            ...
            {filtered.map((st) => {
              ...
              return (
                <div key={st.id} data-sec-row="" style={{ display: "grid", gridTemplateColumns: "1.2fr 2fr 1fr 1.6fr", gap: 12, alignItems: "center", padding: "12px 20px", borderBottom: "1px solid #f5f7fa" }}>
                  ...
                  <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                    <button onClick={() => setMark(st.id, "present")} style={{ border: "1px solid #e5e9f2", borderRadius: 8, padding: "7px 14px", fontSize: 11.8, fontWeight: 600, cursor: "pointer", background: m === "present" ? MARK_TONE.bgP : "#ffffff", color: m === "present" ? MARK_TONE.fgP : "#64748b" }}>Present</button>
                    <button onClick={() => setMark(st.id, "absent")} style={{ border: "1px solid #e5e9f2", borderRadius: 8, padding: "7px 14px", fontSize: 11.8, fontWeight: 600, cursor: "pointer", background: m === "absent" ? MARK_TONE.bgA : "#ffffff", color: m === "absent" ? MARK_TONE.fgA : "#64748b" }}>Absent</button>
                  </div>
                </div>
              );
            })}
      ...
      {tab === "History" && (
        <div>
          ...
          <div data-sec-lift="" style={{ background: "#ffffff", border: "1px solid #e5e9f2", borderRadius: 14, overflow: "hidden" }}>
            ...
            {histError && (
              <div style={{ padding: 40, textAlign: "center", fontSize: 12.2, color: "#b91c1c" }}>
                Attendance history isn&apos;t reachable for this account yet — a pre-existing backend route collision
                (GET /me/attendance also matches a student-only profile controller) blocks this read. Flagged for a
                backend fix; not something this screen can work around on its own.
              </div>
            )}
            {(histRecords?.data ?? []).map((r) => {
              const t = r.status === "absent" ? { bg: "#fef2f2", fg: "#b91c1c" } : { bg: "#ecfdf5", fg: "#047857" };
              return (
