 ...
 * Rows with `onRowClick` but no explicit `hoverableRows` get the reference
 * design's `.hover-lift` treatment automatically (see `onRowClick` doc above).
 */
        ...
        data.map((row, index) => (
          <div
            ...
            className={cn(
              ...
              onRowClick && !hoverableRows && "hover-lift rounded-[10px]",
            )}
            style={{ gridTemplateColumns }}
          >
            {columns.map((col) => (
