  ...
  return (
    <Modal open={open} onClose={handleClose} title="New feedback form" className="max-w-2xl">
      <div className="mb-3.5">
        ...
        <Input value={title} onChange={(e) => setTitle(e.target.value)} maxLength={200} placeholder="e.g. End Semester Faculty Feedback — Odd 2026" />
      </div>
      ...
      <div className="mb-3.5">
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => setTargetKind("sections")}
            className={`h-9 flex-1 rounded-input border text-[12.5px] font-semibold ${
              targetKind === "sections" ? "border-primary bg-accent-100 text-primary" : "border-border-default bg-surface text-body"
            }`}
          >
            Department &amp; sections
          </button>
          <button
            ...
            className={`h-9 flex-1 rounded-input border text-[12.5px] font-semibold ${
              targetKind === "batch" ? "border-primary bg-accent-100 text-primary" : "border-border-default bg-surface text-body"
            }`}
          >
            An entire batch
          </button>
                ...
                {sectionsInDept.map((c) => {
                  ...
                  return (
                    <button
                      ...
                      className={`h-8 rounded-pill border px-3.5 text-[12.5px] font-semibold ${
                        active ? "border-primary bg-primary text-white" : "border-border-default bg-surface text-body"
                      }`}
                    >
                      Section {c.section}
                    </button>
