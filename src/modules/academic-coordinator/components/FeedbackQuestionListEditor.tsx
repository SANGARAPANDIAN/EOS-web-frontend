...
export function FeedbackQuestionListEditor({ questions, onChange, headerExtra }: FeedbackQuestionListEditorProps) {
  ...
  return (
    <div className="mt-3.5 border-t border-border-default pt-3.5">
      <div className="mb-2 flex items-center justify-between">
        <p className="m-0 text-[11px] font-bold tracking-[.4px] text-subtle">QUESTIONS</p>
        <div className="flex items-center gap-3">
          {headerExtra}
          <Button variant="text" onClick={() => onChange([...questions, { question_text: "", question_type: "rating" }])}>
            + Add question
          </Button>
        </div>
      </div>
      <div className="flex flex-col gap-2">
        {questions.map((q, i) => (
          <div key={i} className="flex items-center gap-2">
            <span className="w-[18px] shrink-0 text-[11.5px] text-subtle">{i + 1}.</span>
            <Input
            ...
            <IconButton
              ...
              className={cn(
                "shrink-0 border-danger-border",
                questions.length === 1 ? "cursor-not-allowed text-danger-fg opacity-40" : "text-danger-fg hover:bg-danger-bg",
              )}
            />
          </div>
        ))}
