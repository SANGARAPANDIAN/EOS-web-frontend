...
export function CannotDeleteModal({ open, onClose, label, blockers }: CannotDeleteModalProps) {
  return (
    <Modal
      ...
      subtitle="Something still points at it."
      className="max-w-md"
    >
      <div className="flex gap-3">
        <Icon name="warning" size={20} className="mt-0.5 shrink-0 text-danger-fg" />
        <div>
          <ul className="m-0 flex flex-col gap-1 pl-[18px] text-[13px] text-body">
            {blockers.map((b) => (
              <li key={b}>{b}</li>
            ))}
          </ul>
          <p className="mt-2.5 text-xs text-muted">
            Remove or move those first. This is not a caution — the foreign keys are declared NoAction, so the delete
            would simply fail.
          </p>
        </div>
      </div>
