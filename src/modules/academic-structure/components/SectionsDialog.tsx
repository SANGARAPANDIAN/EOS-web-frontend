  ...
  return (
    <Modal open={open} onClose={onClose} title="Add sections" subtitle={course.name} className="max-w-sm">
      ...
      <div className="mb-3.5">
          ...
          {SECTION_LETTERS.map((letter) => {
            ...
            return (
              <button
                ...
                className={`flex size-11 items-center justify-center rounded-input border text-sm font-bold ${
                  taken
                    ? "cursor-not-allowed border-border-default bg-surface-tint text-subtle"
                    : active
                      ? "border-primary bg-primary text-white"
                      : "border-border-default bg-surface text-body"
                }`}
              >
                {letter}
              </button>
        ...
        {allLettersTaken && <p className="mt-1 text-[11px] text-subtle">A–D already exist for this batch — add more below.</p>}
      </div>
      <div className="mb-3.5">
