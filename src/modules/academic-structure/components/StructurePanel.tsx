...
/** Shared danger-tint hover treatment for a delete IconButton — dims cleanly via `disabled:` instead of branching className in JS. */
const DANGER_ICON_BUTTON_CLASSES =
  "text-danger-fg hover:border-danger-border hover:bg-danger-bg disabled:cursor-not-allowed disabled:text-subtle disabled:hover:border-border-default disabled:hover:bg-surface";
...
export function StructurePanel({
  ...
  const hasBlockingChildren = departmentCourses.length > 0 || departmentClasses.length > 0;
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="rounded bg-surface-muted px-[7px] py-[3px] text-[10.5px] font-bold text-muted">
                {department.code}
              </span>
            </div>
            <div className="mt-2 text-[12.5px] text-muted">
              Head of Department:{" "}
              {hod ? `${hod.first_name} ${hod.last_name}${hod.designation ? ` — ${hod.designation}` : ""}` : "Not assigned"}
              {!readOnly && (
                <Button variant="text" className="ml-2 text-xs" onClick={() => setHodPickerOpen(true)}>
                  {hod ? "Change" : "Assign"}
                </Button>
              )}
            </div>
          ...
          {!readOnly && (
            <div className="flex gap-1.5">
              <IconButton icon="edit" size={30} iconSize={15} onClick={onEditDepartment} title="Edit" />
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-body">Courses</h3>
        {!readOnly && (
          <Button variant="primarySmall" onClick={onAddCourse}>
            + Add course
          </Button>
        )}
      </div>
      ...
      {departmentCourses.length === 0 ? (
        <Card className="border-dashed p-10 text-center">
          ...
          <p className="text-[13px] font-semibold text-body">No courses in {department.code} yet.</p>
          <p className="mx-auto mt-1.5 max-w-[360px] text-xs text-muted">
            A course is the degree — B.E. Computer Science and Engineering, M.E. Structural Engineering. Classes hang
            off it.
          </p>
          {!readOnly && (
            <Button variant="primarySmall" className="mt-3.5" onClick={onAddCourse}>
      ...
      <div className="flex items-start justify-between border-b border-divider px-4 py-3.5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="rounded bg-surface-muted px-1.5 py-0.5 text-[10px] font-bold text-muted">{course.code}</span>
          </div>
        ...
        <div className="flex items-center gap-5">
          ...
          {!readOnly && (
            <div className="flex gap-1.5">
              <IconButton icon="edit" size={28} iconSize={14} onClick={onEdit} title="Edit" />
              {deleteCourse.isPending ? (
                <span className="flex size-7 items-center justify-center">
                  <Spinner size={14} className="text-danger-fg" />
                </span>
              ) : (
                <IconButton
                  icon="delete"
      ...
      <div className="p-4">
        {batchesInUse.length === 0 ? (
          <div className="py-6 text-center">
            ...
            <p className="text-[12.5px] font-semibold text-body">No classes yet.</p>
            <p className="mt-1 text-[11.5px] text-muted">
...
function BatchRow({ batch, classes, course, onEditClass, readOnly = false }: BatchRowProps) {
  ...
  return (
    <div className="rounded-[9px] border border-divider p-3">
      ...
      <div className="grid grid-cols-4 gap-2">
        {classes.map((cls) =>
          readOnly ? (
            <div key={cls.id} className="rounded-lg border border-border-default bg-surface-muted px-2.5 py-2 text-left">
            ...
            <button
              ...
              className="cursor-pointer rounded-lg border border-border-default bg-surface-muted px-2.5 py-2 text-left transition-colors hover:border-border-accent"
            >
              <div className="text-[13px] font-bold text-ink">{cls.section}</div>
              <div className="mt-0.5 text-[10px] text-subtle">
                {cls.current_semester ? `Semester ${cls.current_semester}` : "No semester set"}
        ...
        {nextLetter && (
          <button
            ...
            className="cursor-pointer rounded-lg border border-dashed border-border-default bg-surface px-2.5 py-2 text-left text-subtle transition-colors hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-50"
          >
