...
interface AdminSidebarProps {
  ...
  mobileOpen: boolean;
  onCloseMobile: () => void;
  badges?: Partial<Record<"studentCount" | "facultyCount", ReactNode>>;
  userName?: string;
  userEmail?: string;
}
...
function UserFooter({ collapsed, userName, userEmail }: { collapsed: boolean; userName?: string; userEmail?: string }) {
    ...
    <div className={cn("flex items-center gap-2.5 border-t border-admin-border p-3", collapsed && "justify-center")}>
      ...
      {!collapsed && (
        <div className="min-w-0 flex-1">
          <div className="truncate text-[13px] font-bold text-admin-ink">{userName ?? userEmail ?? " "}</div>
          <div className="text-[12px] text-admin-muted">Admin</div>
        </div>
      )}
      {otherRoles.length > 0 && (
        ...
        <div className={cn("flex items-center gap-3 border-b border-admin-border px-5 pt-5 pb-[18px]", collapsed && "justify-center px-0")}>
          ...
          {!collapsed && (
            <div className="leading-[1.15]">
              ...
              <div className="text-[10.5px] font-semibold text-admin-muted">Admin Console</div>
            </div>
          )}
        </div>
        <NavContent collapsed={collapsed} onToggleCollapsed={onToggleCollapsed} badges={badges} />
        <UserFooter collapsed={collapsed} userName={userName} userEmail={userEmail} />
      </aside>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 flex lg:hidden">
          ...
          <aside className="relative flex h-full w-[264px] flex-col overflow-hidden bg-admin-canvas shadow-admin-modal">
            ...
            <UserFooter collapsed={false} userName={userName} userEmail={userEmail} />
          </aside>
        </div>
      )}
    </>
