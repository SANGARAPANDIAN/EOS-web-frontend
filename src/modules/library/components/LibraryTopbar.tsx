...
/** Search box doubles as a lightweight "jump to page" — filters the nav's flat item list by label. */
export function LibraryTopbar({ onOpenMobileNav }: LibraryTopbarProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!q) return;
    const match = LIBRARY_NAV.flatMap((g) => g.items).find((item) => item.label.toLowerCase().includes(q));
    if (match) {
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center gap-4 border-b border-admin-border bg-admin-canvas/92 px-7 py-3 backdrop-blur-[8px]">
      ...
      <form
        ...
        className="flex h-10 w-full max-w-[420px] items-center gap-2.5 rounded-admin-control border border-admin-border bg-admin-canvas px-3 has-[input:focus]:border-admin-primary"
      >
      ...
      <div className="flex items-center gap-2 rounded-admin-pill border border-admin-border bg-admin-tint-strong px-3.5 py-2 text-[13px] font-semibold text-admin-primary-deep">
        <Icon name="local_library" size={17} />
        Library · Institution
      </div>
    </header>
  );
