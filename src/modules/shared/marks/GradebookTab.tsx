...
/**
 * A per-student × per-exam marks grid for the subjects the caller
 * personally teaches — shared by HoD's own Subject Records page and the
 * real Faculty/Advisor portal's Subject Records page, backed by the same
 * `/hod/my-class/subject-records` endpoint (kept at that URL for now;
 * already `@Roles(FACULTY, HOD)`, resolved via the caller's own faculty
 * row either way — the HoD-flavored URL is a known, harmless naming wart,
 * not a functional restriction). Previously HoD-only, forked verbatim into
 * this shared location so any teaching faculty gets the same view.
 */
